...
/****************************************************************/
#include "SwitchingFunctionMaterialNoOver.h"

template <>
InputParameters
validParams<SwitchingFunctionMaterialNoOver>()
{
...
SwitchingFunctionMaterialNoOver::SwitchingFunctionMaterialNoOver(const InputParameters & parameters)
  : OrderParameterFunctionMaterial(parameters), _h_order(getParam<MooseEnum>("h_order"))
{
}

void
SwitchingFunctionMaterialNoOver::computeQpProperties()
{
  Real n = _eta[_qp];
  n = n > 1 ? 1 : (n < 0 ? 0 : n);
  ...
  if (n<0.0)
  {
    _prop_f[_qp] = -_prop_f[_qp];
    _prop_df[_qp] = -_prop_df[_qp];
    _prop_d2f[_qp] = -_prop_d2f[_qp];
  }
  // if (n>1.0)
  // {
  //   _prop_f[_qp] = 2.0-_prop_f[_qp];
  //   _prop_df[_qp] = -_prop_df[_qp];
  //   _prop_d2f[_qp] = -_prop_d2f[_qp];
  // }

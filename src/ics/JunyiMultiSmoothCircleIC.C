...
#include "JunyiMultiSmoothCircleIC.h"
#include "MooseMesh.h"
...
{
  InputParameters params = validParams<JunyiSmoothCircleBaseIC>();
...
JunyiMultiSmoothCircleIC::JunyiMultiSmoothCircleIC(const InputParameters & parameters) :
    JunyiSmoothCircleBaseIC(parameters),
    ...
    _variation_invalue(parameters.get<Real>("variation_invalue"))
{
  ...
  _variation.resize(_numbub);

  for (unsigned int i = 0; i < _numbub; i++)
  {
    // Vary bubble radius
    // QZ
    _variation[i] = - _variation_invalue + 2.0 * _randomVal.rand(_tid) * _variation_invalue;
  }
}

void
JunyiMultiSmoothCircleIC::initialSetup()
{
...
void
JunyiMultiSmoothCircleIC::computeCircleRadii()
{
...
void
JunyiMultiSmoothCircleIC::computeCircleCenters()
{
  ...
  for (unsigned int i = 0; i < _numbub; ++i)
  {
    ...
    if (num_tries == _max_num_tries)
      mooseError("Too many tries in JunyiMultiSmoothCircleIC");

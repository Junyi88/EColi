...
template <>
InputParameters
validParams<MskMeltTimeDerivative>()
{
...
MskMeltTimeDerivative::MskMeltTimeDerivative(
  ...
  : DerivativeMaterialInterface<JvarMapKernelInterface<TimeDerivative>>(parameters),
    ...
    _dEta_dot(coupledDotDu("Eta")), _Mask(getMaterialProperty<Real>("Mask"))
{
...
Real
MskMeltTimeDerivative::computeQpResidual()
{
...
Real
MskMeltTimeDerivative::computeQpJacobian()
{
  if (_Mask[_qp]<0.5)
    return 0.0;
...
Real
MskMeltTimeDerivative::computeQpOffDiagJacobian(unsigned int jvar)
{

...
template <>
InputParameters
validParams<HeatConductionTimeDerivative2>()
{
...
HeatConductionTimeDerivative2::HeatConductionTimeDerivative2(const InputParameters & parameters)
  : TimeDerivative(parameters),
    _specific_heat(getMaterialProperty<Real>("specific_heat")),
    _density(getMaterialProperty<Real>("density_name")), _Mask(getMaterialProperty<Real>("Mask"))
{
}
